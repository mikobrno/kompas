-- Add color selection support for categories and expose it via RPC responses
ALTER TABLE categories
  ADD COLUMN color_hex text NOT NULL DEFAULT '#f05a28';

ALTER TABLE categories
  ADD CONSTRAINT categories_color_hex_format CHECK (color_hex ~* '^#[0-9A-F]{6}$');

-- Drop old definition so we can extend the return type with color column
